...
import { Shield, Clock, CheckCircle, ChevronRight, ArrowRight, Star, Phone } from 'lucide-react';
import { useAuthStore } from '../store/authStore';
import { useBookingStore } from '../store/bookingStore';
import { AddressSearch } from '../components/AddressSearch';
import { TrustBadgeStrip, getAvailabilityMessage } from '../components/TrustBadgeStrip';
        ...
        <div className="relative max-w-7xl mx-auto px-6 w-full flex flex-col lg:flex-row items-center gap-8 lg:gap-16 py-8 lg:py-24">
          ...
          <div className="flex-1 max-w-xl">
            ...
            <div className="border-2 border-[#008060] bg-white max-w-md shadow-[0_0_0_4px_rgba(0,128,96,0.08)]">
              {/* $20 promo banner */}
              <div className="bg-[#008060] px-4 py-2 flex items-center gap-2">
                <span className="text-white text-base font-black">$20</span>
                <div>
                  <p className="text-white text-[11px] font-black uppercase tracking-wide leading-tight">Off your first booking</p>
                  <p className="text-white/65 text-[9px] font-mono leading-tight">New customers · limited time</p>
                </div>
              </div>
              ...
              <div className="px-4 pb-3 flex items-center justify-between">
                ...
                <p className="text-[10px] font-mono text-black/45 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#008060] inline-block" />
            ...
            <div className="mt-4 flex items-center gap-4 flex-wrap">
              ...
              <a href="tel:12403660377" className="flex items-center gap-1.5 text-black/50 hover:text-black text-[12px] font-mono transition-colors">
                <Phone className="w-3 h-3" />(240) 366-0377
              </a>
            </div>
          </div>
      ...
      <section className="py-20 bg-black border-t border-black/10">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10">
            {TESTIMONIALS.map(({ name, role, rating, quote, source }) => (
              <div key={name} className="bg-black p-8 flex flex-col gap-5">
                ...
                <div className="flex items-center gap-1.5 pt-3 border-t border-white/10">
                  {source === 'google' ? (
                    <>
                      <svg width="12" height="12" viewBox="0 0 24 24" aria-label="Google" className="flex-shrink-0">
                        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/>
                        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
                      </svg>
                      <span className="text-white/35 text-[10px] font-mono">Google Review</span>
                    </>
                  ) : (
                    <>
                      <svg width="12" height="12" viewBox="0 0 24 24" aria-label="Yelp" fill="#FF1A1A">
                      ...
                      <span className="text-white/35 text-[10px] font-mono">Yelp Review</span>
                    </>
                  )}
                </div>
              </div>
